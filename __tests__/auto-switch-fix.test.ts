...
describe("Auto-Switch Fix", () => {
  ...
  test("Should sort branches by timestamp correctly", () => {
    const branches = [
      { id: "main", name: "Main" },
      { id: "branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046000000_abc", name: "Branch 1" },
      { id: "branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046002000_def", name: "Branch 2" },  
      { id: "branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046001000_ghi", name: "Branch 3" },
    ]
    ...
    const sorted = branchesWithTimestamps.sort((a, b) => b.timestamp - a.timestamp)
    const newestBranch = sorted[0]?.branch

    // Should get the branch with timestamp 1750046002000 (newest)
    expect(newestBranch?.id).toBe("branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046002000_def")
  })

  test("Should handle branches with different message IDs", () => {
    const branches = [
      { id: "main", name: "Main" },
      { id: "branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046000000_abc", name: "Branch A" },
      { id: "branch_k8g9h1j2l3m4n5o6p7q8r9s0_1750046001000_def", name: "Branch B" },  
    ]
    ...
    expect(newestBranch?.id).toBe("branch_k8g9h1j2l3m4n5o6p7q8r9s0_1750046001000_def")
  })
  ...
  test("Should handle malformed branch IDs gracefully", () => {
    const branches = [
      ...
      { id: "branch_j97n4k3h2d5e6f7g8h9i0j1k_1750046000000_abc", name: "Valid" },
    ]
