...
export function useMessages(options: UseMessagesOptions = {}) {
	...
	const createThreadAndSendMutation = useMutation(
	...
	const optimisticSendMessage = useMutation(
		api.messages.send,
	...
	return {
		...
		userMessageCount: messages.filter((m) => m.message.messageType === "user")
			.length,
		assistantMessageCount: messages.filter(
			(m) => m.message.messageType === "assistant",
		).length,
	};
}
