...
// Export Zod schemas for AI SDK runtime validation
	...
	output: z.object({
		...
		results: z.array(
			z.object({
				...
				relevanceRank: z.number().optional(),
...
// Export Convex validators for database storage
export const webSearchValidators = {
	input: v.object({
		query: v.string(),
	}),
	output: v.object({
		...
		results: v.array(
			v.object({
				...
				relevanceRank: v.optional(v.number()),
...
export type WebSearchInput = z.infer<typeof webSearchSchemas.input>;
export type WebSearchOutput = z.infer<typeof webSearchSchemas.output>;
...
export function createWebSearchTool() {
	return tool({
		description: webSearchMetadata.fullDescription,
		inputSchema: webSearchSchemas.input,
		execute: async ({ query }) => {
