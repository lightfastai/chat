...
export class MessageService {
  ...
  async createStreamingMessage(
    ...
    const provider = getProviderFromModelId(this.modelId)
  ...
  async buildConversationMessages(
    ...
    const recentMessages = await this.ctx.runQuery(
      internal.messages.getRecentContext,
      { threadId: this.threadId },
    )

    const provider = getProviderFromModelId(this.modelId)
    const systemPrompt = createSystemPrompt(this.modelId, webSearchEnabled)
  ...
  async streamResponse(
    ...
    const provider = getProviderFromModelId(this.modelId)
    const aiClient = createAIClient(this.modelId, userApiKeys)
    ...
    if (provider === "anthropic" && isThinkingMode(this.modelId)) {
    ...
    for await (const chunk of result.textStream) {
      if (chunk) {
        ...
        await this.ctx.runMutation(internal.messages.appendStreamChunk, {
          messageId,
          chunk,
          chunkId,
        })
  ...
  async updateThreadUsage(
    ...
    await this.ctx.runMutation(internal.messages.updateThreadUsageMutation, {
      threadId: this.threadId,
      usage: {
        promptTokens,
        completionTokens,
   ...
   */
  async clearGenerationFlag() {
    await this.ctx.runMutation(internal.messages.clearGenerationFlag, {
      threadId: this.threadId,
    })
  }
}
