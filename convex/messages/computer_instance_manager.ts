...
export class ComputerInstanceManager {
  ...
  private async initializeSDK(): Promise<LightfastComputerSDK> {
    ...
    if (!env.FLY_API_TOKEN || !env.FLY_APP_NAME) {
      throw new Error("Computer SDK requires FLY_API_TOKEN and FLY_APP_NAME environment variables")
    }
   ...
   */
  async getOrCreateInstance(): Promise<Instance> {
    // Check cache first
    const cachedInstance = instanceCache.get(this.threadId)
    if (cachedInstance && cachedInstance.status === "running") {
      console.log(`Using cached instance for thread ${this.threadId}: ${cachedInstance.id}`)
    ...
    if (listResult.isOk()) {
      ...
      const threadInstances = listResult.value.filter(
        (i) => i.status === "running" && 
        (i.name === `git-analysis-${this.threadId}` || i.metadata?.threadId === this.threadId)
      )
    ...
    const createOptions: CreateInstanceOptions = {
      name: `git-analysis-${this.threadId}`,
