...
export function CreditBalanceCard({ balance }: CreditBalanceCardProps) {
  ...
  const usagePercentage = (balance.periodUsage / balance.monthlyAllocation) * 100
  const remainingPercentage = (balance.balance / balance.monthlyAllocation) * 100
  const periodEnd = new Date(balance.periodEnd)
  const daysUntilReset = Math.ceil((balance.periodEnd - Date.now()) / (1000 * 60 * 60 * 24))
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            Resets in {daysUntilReset} day{daysUntilReset !== 1 ? 's' : ''} ({periodEnd.toLocaleDateString()})
          </span>
        </div>
        ...
        {balance.balance < 50 && (
          <div className="p-3 bg-orange-50 border border-orange-200 rounded-md">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-orange-800 font-medium">Running Low</span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2 border-t">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-blue-600">{balance.periodUsage}</p>
    ...
    <Card>
      ...
      <CardContent>
        <div className="text-center py-6">
          ...
          <p className="text-sm text-muted-foreground">
            Subscribe to a plan to get credits and start chatting with AI models.
          </p>
        </div>
      </CardContent>
    </Card>
